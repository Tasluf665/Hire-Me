...
import OrderTrackIndecator from "../../Components/CartScreen/OrderTrackIndecator";
import ContactCard from "../../Components/CartScreen/ContactCard";
import CustomeActivityIndicator from "../../Components/CustomeActivityIndicator";

export default function OrderTrackMainScreen() {
        ...
        const getData = async () => {
            try {
                const response = await fetch(
                    `${API_URL}/api/users/orders/${orderId}`,
                    {
                        headers: {
                ...
                if (!result.error) {
                    if (result.data.technicianId) {
                        const res = await fetch(
                            `${API_URL}/api/technicians/${result.data.technicianId}`,
                            {
                                headers: {
                                    "Content-Type": "application/json",
                                    "x-auth-token": token,
